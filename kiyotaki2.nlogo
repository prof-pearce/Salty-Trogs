...
to init-model
  ...
  create-traders num-traders / 3; num-sugar-consumers
  [
    set good-produced 0  ; I produce salt
    set good-consumed 1  ; I consume sugar
    set good-held good-produced
    set color green
    set utility 0
    set partner nobody
    ;set is-speculative sugar-consumer-is-speculative
    set storage-costs [.1 .2 .3] ; obeys constraint
    set production-cost .3
    set marginal-utility 1
    init-trader
  ]
  create-traders num-traders / 3; num salt consumers
  [
    set good-produced 1  ; I produce sugar
    set good-consumed 2  ; I consume spice
    set good-held good-produced
    set color red
    set utility 0
    set partner nobody
    ;set is-speculative salt-consumer-is-speculative
    set storage-costs [.1 .2 .3]
    set production-cost .3
    set marginal-utility 1
    init-trader
  ]
  create-traders num-traders / 3; num-spice-producers
  [
    set good-produced 2  ; I produce spice
    set good-consumed 0  ; I consume salt
    set good-held good-produced
    set color blue
    set utility 0
    set partner nobody
...
to-report will-trade? [candidate]
  ...
  [
     ...
     [
      ...
      report speculate? candidate-good
     ]
  ]
end

to-report speculate? [good]
  let diff1 item good storage-costs - item good-held storage-costs
  let factor marginal-utility * beta / 3
  let diff2 factor * ((prob good good-consumed) - ( prob good-held good-consumed))
  report diff1 > diff2
end

to-report prob [trader-type good]
  ifelse good = trader-type
  [
    report 0
  ]
  [
     let a count traders with [good-consumed = trader-type and good-held = good]
     report a / count traders
  ]
end
...
to update-utility
  ...
  [
    set good-held good-produced
    set utility marginal-utility - production-cost - item good-held storage-costs
  ]
  [
    set utility utility - item good-held storage-costs
  ]
...
My interpretation of Kiyotaki & Wright's "On Money as a Medium of Exchange", 1989.


## HOW IT WORKS


I've been looking over Kiyotaki and Wright trying to decide what purpose a NetLogo model might serve, if only to help understand the paper. 

I developed two similar models in which there are three types of traders: Type-0, 1, and 2 and three types of goods: Good-0, 1, and 2. Type-i consumes Good-i and produces Good-j for j = (i + 1) mod 3. (This is their model A.) 

During each update cycle traders are paired and must decide if they want to trade or not. The decision is made using an optimal trading strategy that will produce the maximum lifetime good for each trader. The paper finds Nash equilibria in terms of various possible storage costs. It turns out that in the optimal strategy Type-i always trades for Good-i but never trades Good-j for Good-j. That only leaves the question: should Type-i trade Good-j for Good-k where k != j != i. In other words, should Type-i trade for a good he can't consume. K & W figure out the storage cost rangess for when the answer is yes and when it is no. They then show these strategies are Nash equilibria. 
...
* In all other cases A trades with B if A is speculative. 
...
Users can set the strategy used by each type of trader. This can be thought of as the trading strategy used by the trader. 
